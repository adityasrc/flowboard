...
export function LandingHeader() {
  ...
  // 1. Scroll Listener with passive flag for 60FPS mobile scrolling
  useEffect(() => {
    const handleScroll = () => {
  ...
  // 2. Auth Listener with safe browser storage access
  useEffect(() => {
    const checkAuth = () => {
      try {
        ...
        // Fallback for Safari Incognito or restricted iframe embeds where storage is blocked
        setIsLoggedIn(false);
      } finally {
        setIsLoading(false);
      }
  ...
  // 3. Prevent background page scrolling when the mobile menu is open
  useEffect(() => {
    if (isMobileMenuOpen) {
    ...
    <header className={`fixed top-0 z-50 w-full transition-all duration-300 ${navBackgroundClass}`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6">
        ...
        <nav className="hidden md:flex items-center gap-8 lg:gap-10">
          ...
          <Link href="https://github.com/adityasrc/flowboard" target="_blank" rel="noreferrer" className="text-[13px] font-medium text-slate-500 hover:text-black transition-colors">
            GitHub
          </Link>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 top-[60px] z-40 bg-white/95 backdrop-blur-lg animate-in fade-in slide-in-from-top-4 duration-300">
          <nav className="flex flex-col p-8 gap-8">
            ...
            <Link href="/docs" onClick={() => setIsMobileMenuOpen(false)} className="text-xl font-semibold text-slate-900">Docs</Link>
            <Link href="https://github.com/adityasrc/flowboard" target="_blank" rel="noreferrer" onClick={() => setIsMobileMenuOpen(false)} className="text-xl font-semibold text-slate-900">GitHub</Link>
